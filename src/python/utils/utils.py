...
def get_label_users(to_age_group=True):
    ...
    LOGGER.info(
        'the number of user_id who have labels for gender and age after removing duplicates {}'.format(len(df_cats)))
...
def get_optimal_threshold(Y_test, Y_prob, distribution, specific_classes=None, sample_weight=None,
    ...
    while iteration < 100:
        ...
        for k in range(len(Y_prob)):  # iterate all samples
            ...
            corresponding_pos_for_segment = [
                find_corresponding_pos_of_prob(threshold, prob) if prob >= segment_threshold[i]
...
def compute_precision_recall_volume_BV(Y_test, Y_prob, nclass, distribution, sample_weight=None):
    ...
    for (i, T) in enumerate(threshold):
        for c in range(nclass):
            Y_pred = (Y_prob[:, c] >= T).astype(np.int)
            volume[i][c] = np.sum(Y_pred) * 1.0 / len(Y_test) if sample_weight is None else np.sum(
                Y_pred * sample_weight) / np.sum(sample_weight)
...
def show_TPRV(Y_test, Y_prob, nclass, distribution, sample_weight=None, known_type='threshold',
    ...
    '''
    nclass = Y_prob.shape[1]
    threshold, precision, recall, volume, _, _ = compute_precision_recall_volume_BV(Y_test, Y_prob, distribution,
                                                                                    sample_weight)
